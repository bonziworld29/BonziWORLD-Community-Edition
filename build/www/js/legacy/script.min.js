...
function login() {
  ...
  if($("#login_name").val().includes("\"") === true) { return socket.disconnect() && $("#page_error").hide() }
	if($("#login_name").val().includes("'") === true) { return socket.disconnect() && $("#page_error").hide() }
	if($("#login_name").val().includes("&") === true) { return socket.disconnect() && $("#page_error").hide() }
	if($("#login_name").val().includes("#") === true) { return socket.disconnect() && $("#page_error").hide() }
...
function login2() {
  // redo login if safe mode
  if (safeMode) {
    socket.emit("login", { name: $("#login_name2").val(), room: $("#login_room2").val() });

  } else {
    socket.emit(window.bonzi_guid + "_login_" + window.testguid, { name: $("#login_name2").val(), room: $("#login_room2").val() });
  ...
  Bonzi = (function() {
    function constructor(id, userPublic) {
      ...
      _classCallCheck(this, constructor),
        ...
        (this.run = !0),
        (this.mute = !1),
        (this.eventTypeToFunc = { anim: "updateAnim", html: "updateText", text: "updateText", idle: "updateIdle", add_random: "updateRandom", clearText: "clearDialog" }),
      ...
      $.contextMenu({
				...
				items: {
					...
					/*
					  ...
					  shadowban: {
						...
						name: "Silent Kick",
						callback: function() {
						  socket.emit("command", { list: ["shadowkick", d.id] })
						},
					  },
					  hypnotize: {
						disabled: function() {
				  ...
				  givepopeto: {
					...
					name: "Popeify",
					callback: function() {
					  socket.emit("command", { list: ["givepopeto", d.id] })
					},
				  },
				  givegodto: {
					disabled: function() {
					...
					name: "Godify",
					callback: function() {
					  socket.emit("command", { list: ["givegodto", d.id] })
					},
				  }
				},
			  }
		...
		var theDiv = document.getElementById("logs");
		theDiv.innerHTML += "<br><br>" + this.userPublic.name + " joined the room.";
        setTimeout(function() {
          if (d.color == "diogo") {
      ...
      _createClass(constructor, [
          ...
          value: function(e) {
            if (this.drag) {
              if (window.bonzi_guid == this.id) {
                ...
                this.dragged = true; 
                socket.emit("command", { list: ["move", e.pageX - this.drag_start.x, e.pageY - this.drag_start.y, this.dragged] });
        ...
        },
        {
          ...
          value: function() {
			...
			if (allowCrossColors == true) {
				...
				if (this.color == "empty" && this.userPublic.color_cross != 'none') {  
					this.$canvas.css("background-image", `url("/img/bonzi/bonzi.webp")`);
				} else {
					this.$canvas.css("background-image", `url("/img/bonzi/${this.color}.webp")`);
				}
        ...
        },
        {
          ...
          value: function(text, say, allowHtml) {
            ...
            this.$dialogCont
            [allowHtml ? "html" : "text"](allowHtml ? twemoji.parse(text) : text)
          ...
          if (this.userPublic.voice != "sam" && this.userPublic.voice != "broken" && this.userPublic.voice != "espeak" && this.userPublic.voice != "mike" && this.userPublic.voice != "mary") {
			...
			if (this.userPublic.voice == "broken") {
				var say2 = say.replaceAll(/ /gi, "' ").replaceAll(/'s/gi, " s").replaceAll(/]]/gi, "").replaceAll(/!/gi, "]]! [['").replaceAll(/\?/gi, "]]? [['")
				  speak.play("[['" + say2, {
					...
					var say2 = say.replaceAll(/soi/gi,"[[_^_zh]] swoier [[_^_en]]").replaceAll(/soy/gi,"[[_^_zh]] swoier [[_^_en]]").replaceAll(/~/gi,"!").replaceAll(/!/gi,"! [[_^_en]]").replaceAll(/,/gi,", [[_^_en]]").replaceAll(/./gi,". [[_^_en]]").replaceAll(/:/gi,": [[_^_en]]")
            ...
            var theDiv = document.getElementById("logs");
            theDiv.innerHTML += "<br><br>" + this.userPublic.name + ""
        ...
        },
        {
          ...
          value: function(callback) {
			...
			var theDiv = document.getElementById("logs");
			theDiv.innerHTML += "<br><br>	" + this.userPublic.name + " left the room.";
          },
        },
        {
          key: "deconstruct",
        ...
        },
        {
          key: "bass",
          value: function(target) {
            this.runSingleEvent([{ type: "text", text: "Hey, " + target + "!" }, { type: "text", text: "You're a fucking bass!", say: "your a fucking bass!" }, { type: "clearText" }, { type: "anim", anim: "laugh_fwd", ticks: 25 }, { type: "idle" }]);
          },
        },
        {
          key: "beggar",
          value: function(target) {
            this.runSingleEvent([{ type: "text", text: "Hey, " + target + "!" }, { type: "text", text: "You're a fucking beggar!", say: "your a fucking beggar!" }, { type: "anim", anim: "laugh_fwd", ticks: 25 }, { type: "idle" }]);
          },
        },
        {
          key: "kiddie",
        ...
        },
        {
          ...
          value: function(target) {
            this.runSingleEvent([
              ...
              { type: "text", text: "You're a fucking Logo Kid!", say: "your a fucking logo kid!" },
        ...
        },
        {
          key: "goTard",
          value: function(target) {
            this.runSingleEvent([{ type: "text", text: "Hey, " + target + "!" }, { type: "text", text: "You're a fucking Go!Tard!", say: "your a fucking Go Tard!" }, { type: "anim", anim: "laugh_fwd", ticks: 25 }, { type: "idle" }]);
          },
        },
        {
          key: "forcer",
  ...
  BonziData = {
    ...
    pass_idle: ["gone"],
        ...
        event_list_behh_open: [
            ...
            [
                { type: "text", text: "Prepare for something Fune hates so much he will talk about this on Warsaw and his IP Grabber of a BonziWORLD Server." },
                { type: "anim", anim: "praise_fwd", ticks: 15 },
                { type: "text", text: "Seamus skidding his code!" },
            ...
            [
                { type: "text", text: "Wanna hear me spam behh?" },
                { type: "text", text: "No?" },
                { type: "text", text: "That's ok. I didn't really want to do this anyway" },
            ],
            [
            ...
            [{ type: "text", text: "The behh god wants me to tell a edited version of bonzi.lol's god awful jokes." }],
        ...
        event_list_behh_end: [
            [
                { type: "text", text: "You know {NAME}, a good behh behhs." },
                { type: "text", text: "And you behhing behh. Thanks." },
            ],
            [{ type: "text", text: "Where do I come up with behh? My behh?" }],
            [
                { type: "text", text: "Do I behh you, {NAME}? Am I behh? Do I make you behh?" },
                { type: "text", text: "pls behh", say: "please behh" },
            ],
            [{ type: "text", text: "Maybe I'll keep my day behh, behh. behh didn't accept behh." }],
            [
                { type: "text", text: "behh is the best behh!" },
                { type: "text", text: "Apart from behh." },
            ],
            [
            ...
            [
                { type: "text", text: "Don't judge me on my sense of behh alone." },
                { type: "text", text: "Help! I'm behh!" },
            ],
        ],
    ...
    event_list_joke_mid: [
      ...
      [
        { type: "text", text: "Prepare for something Fune will get pissed off from..." },
        { type: "text", text: "Touching grass!" },
      ...
      [
        ...
        { type: "text", text: "SHIT CUZ- Actually no, fuck the person who wrote this. So, just explain yourself for your existence." },
...
if (window.location.protocol == "https:") {
  socket = io("https://" + hostname + ":" + window.location.port,{transports: ['websocket'], upgrade: false});
} else {
  socket = io("http://" + hostname + ":" + window.location.port,{transports: ['websocket'], upgrade: false});
}
usersPublic = {},
  bonzis = {},
  debug = !0;
...
function reconnect() {
	if (!window.reconnecting) {
		...
		setTimeout(function(){
			...
			// redo login
			socket = io("//" + hostname + ":" + window.location.port,{query:{ channel: "bonziuniverse-nocaptcha" }, transports: ['websocket'], upgrade: false});
